...
both inline the wasm bytes. The helper wires the loader into both bundlers, so the wrapped config builds under Turbopack
(`next build`, the Next.js 16 default) and webpack (`next build --webpack`) alike. Only the inlined-bytes path runs under
Turbopack; the asset-emitting modes need `emitFile`, which Turbopack does not expose. The Edge runtime is unsupported on
