...
bundlers, so the wrapped config builds under Turbopack and webpack alike, on Next.js 14 through 16. The helper reads the
version of the Next.js you run and writes the Turbopack settings under the key that release accepts. See the
[Next.js example](../examples/next) for the command each release takes. The asset-emitting modes (`web.asyncLoading`,
`node.bundle: false`) still need `emitFile`, which Turbopack does not expose, so the helper leaves them out of the Next
path.
