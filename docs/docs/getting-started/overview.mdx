...
Next.js uses the Webpack loader under the hood through the `withRustWasm` helper: the server pass builds `node`, the
client pass builds `web`, and both inline the wasm bytes. The Edge runtime is rejected with a clear build-time error
(Edge cannot instantiate wasm from inlined bytes). Next.js 16 defaults to Turbopack, so opt out with `next build
--webpack`.
